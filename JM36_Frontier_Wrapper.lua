...
--[[ Init - Startup ]]
ColorDefault() os_execute("cls && title JM36 Frontier Wrapper")
ColorBlue() Print("\n", string_format("[ JM36 Frontier Wrapper ] - %s - Wrapper Started", os_date()), "\n") ColorDefault()
...
return t end
local config, configFile = {}, io_open("JM36_Frontier_Wrapper.ini")
if configFile then
...
if not config_FrontierDirGTA then
	_config_FrontierDirGTA = io_popen("powershell [Environment]::GetFolderPath([Environment+SpecialFolder]::MyDocuments)")
...
string_endsWith = nil
local config_DisableAutomaticFrontierLauncher = config.DisableAutomaticFrontierLauncher~="false"
local config_FrontierLauncherPath
if not config_DisableAutomaticFrontierLauncher then
	config_FrontierLauncherPath = config.FrontierLauncherPath or config_FrontierDirGTA.."Launcher.exe"
	os_execute(string_format('mklink /H "%sJM36_Ptr_LauncherF.exe" "%s"  > nul 2> nul', config_FrontierDirGTA, config_FrontierLauncherPath))
	config_FrontierLauncherPath = config_FrontierDirGTA.."JM36_Ptr_LauncherF.exe"
end
local config_PreserveLogs = config.PreserveLogs~="false"
local config_PreservePlayers = config.PreservePlayers~="false"
local config_DisableWrapperSoloPause = config.DisableWrapperSoloPause~="false"
...
local IsOpen_FAN, IsOpen_GTA
local function IsOpenUpdate()
	local _IsOpen_FAN, _IsOpen_GTA = io_popen('tasklist | findstr /B "JM36_Ptr_LauncherF.exe"'), io_popen('tasklist | findstr /B GTA5.exe')
	IsOpen_FAN, IsOpen_GTA = string_find(_IsOpen_FAN:read("*a"), "JM36_Ptr_LauncherF.exe"), string_find(_IsOpen_GTA:read("*a"), "GTA5.exe")
	_IsOpen_FAN:close() _IsOpen_GTA:close()
end



--[[ Launch Frontier Launcher if game is open and launcher is not open ]]
local function LaunchFrontier()
	IsOpenUpdate()
	if IsOpen_GTA and not IsOpen_FAN then
		os_execute(string_format('start "" "%s"', config_FrontierLauncherPath))
		ColorBlue() Print("\n", string_format("[ JM36 Frontier Wrapper ] - %s - Wrapper Launched Frontier Launcher", os_date()), "\n") ColorDefault()
	end
end
...
--[[ Core/Loop ]]
local logFile, WasOpen_GTA
while true do
	if IsOpen_GTA then
		if not logFile then
			WasOpen_GTA = true
			
			ColorGreen() Print("\n", string_format("[ JM36 Frontier Wrapper ] - %s - Wrapper Found Grand Theft Auto V", os_date()), "\n") ColorDefault()
			...
			while IsOpen_GTA and not logFile do
				logFile = io_open(config_FrontierDirGTA.."Frontier.log")
				IsOpenUpdate()
			end
			
			if not config_DisableAutomaticFrontierLauncher and IsOpen_FAN then
				os_execute'taskkill /IM "JM36_Ptr_LauncherF.exe" /F > nul 2> nul'
				ColorBlue() Print("\n", string_format("[ JM36 Frontier Wrapper ] - %s - Wrapper Closed Frontier Launcher", os_date()), "\n") ColorDefault()
			end
		end
		if logFile then
	...
	if not IsOpen_GTA and logFile then
		ColorYellow() Print("\n", string_format("[ JM36 Frontier Wrapper ] - %s - Wrapper Lost Grand Theft Auto V", os_date()), "\n") ColorDefault()
		...
		if config_PreserveLogs or config_PreservePlayers then
			...
			if config_PreservePlayers then
				os_execute(string_format('copy "%sPlayer Manager\\Players.json" "%sLogs\\Players-%s.json" > nul 2> nul', config_FrontierDirGTA, config_FrontierDirGTA, TimeString))
			end
		end
	end
	if not IsOpen_GTA and WasOpen_GTA then
		WasOpen_GTA = false
		if config_DisableWrapperSoloPause then
			ColorYellow() Print("\n", string_format("[ JM36 Frontier Wrapper ] - %s - Wrapper Running Solo", os_date()), "\n") ColorDefault()
		else
			ColorYellow() Print("\n", string_format("[ JM36 Frontier Wrapper ] - %s - Wrapper Running Solo | Press [ENTER] To Recommence", os_date()), "\n") ColorDefault()
			if not io_read() then os_exit() end
			ColorGreen() Print("\n", string_format("[ JM36 Frontier Wrapper ] - %s - Wrapper Resumed", os_date()), "\n") ColorDefault()
		end
	end
end